...
def report(
    ...
    top_k_categories: int = typer.Option(
        5, "--top-k-categories", help="Top-K значений для категориальных признаков."
    ),
    title: str = typer.Option(
        "EDA-отчёт", "--title", help="Заголовок отчёта (первая строка # ... в report.md)."
    ),
    ...
    # 2) Дополнительные флаги качества (новые эвристики)
    quality_flags = compute_quality_flags(
        summary,
        missing_df,
        df,
        high_cardinality_threshold=50,  # осмысленный дефолт
        zero_share_threshold=0.5,
    )

    # 3) Проблемные колонки по пропускам
    problem_missing = (
        missing_df[missing_df["missing_share"] >= float(min_missing_share)]
    ...
    with md_path.open("w", encoding="utf-8") as f:
        ...
        f.write("### Дополнительные эвристики\n\n")
        ...
        f.write(f"  - Порог unique >= **{quality_flags['high_cardinality_threshold']}**\n")
        if quality_flags["high_cardinality_columns"]:
            f.write(f"  - {', '.join(quality_flags['high_cardinality_columns'])}\n")
        f.write(f"- Много нулей в числовых: **{quality_flags['has_many_zero_values']}**\n")
        f.write(f"  - Порог доли нулей >= **{quality_flags['zero_share_threshold']:.0%}**\n")
        ...
        if problem_missing.empty:
            f.write("Колонок выше порога не найдено.\n\n")
        else:
            f.write("Колонки с долей пропусков выше порога:\n\n")
            f.write(problem_missing.to_string())
            f.write("\n\n")
