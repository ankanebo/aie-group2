...
def compute_quality_flags(
    ...
    *,
    ...
    Новые эвристики:
    - has_constant_columns: есть ли колонки, где все значения одинаковые (кроме полностью пустых)
    - has_high_cardinality_categoricals: есть ли категориальные признаки с большим числом уникальных
    - has_many_zero_values: есть ли числовые колонки с большой долей нулей

    Возвращает словарь флагов + вспомогательные списки колонок.
    """
    flags: Dict[str, Any] = {}
    ...
    # 2) Высокая кардинальность категориальных
    high_cardinality_columns: List[str] = []
    for col in df.columns:
        s = df[col]
        is_cat = ptypes.is_object_dtype(s) or isinstance(s.dtype, pd.CategoricalDtype)
        if not is_cat:
            continue
        uniq = int(s.nunique(dropna=True))
        if uniq >= high_cardinality_threshold:
    ...
    # Интегральный скор (простая эвристика)
    score = 1.0
...
def flatten_summary_for_print(summary: DatasetSummary) -> pd.DataFrame:
    """
    Превращает DatasetSummary в табличку для удобного вывода.
    """
